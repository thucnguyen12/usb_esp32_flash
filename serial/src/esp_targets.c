...
typedef esp_loader_error_t (*read_spi_config_t)(void *config, uint32_t efuse_base, uint32_t *spi_config);
...
static esp_loader_error_t spi_config_esp32(void *config, uint32_t efuse_base, uint32_t *spi_config);
static esp_loader_error_t spi_config_esp32xx(void *config, uint32_t efuse_base, uint32_t *spi_config);
...
esp_loader_error_t loader_detect_chip(void *config, target_chip_t *target_chip, const target_registers_t **target_data)
{
    uint32_t magic_value;
    RETURN_ON_ERROR(esp_loader_read_register(config, CHIP_DETECT_MAGIC_REG_ADDR,  &magic_value));
...
esp_loader_error_t loader_read_spi_config(void *config, target_chip_t target_chip, uint32_t *spi_config)
{
    ...
    return target->read_spi_config(config, target->efuse_base, spi_config);
}
...
static esp_loader_error_t spi_config_esp32(void *config, uint32_t efuse_base, uint32_t *spi_config)
{
    *spi_config = 0;
    ...
    uint32_t reg5, reg3;
    RETURN_ON_ERROR(esp_loader_read_register(((esp_loader_config_t*)config), efuse_word_addr(efuse_base, 5), &reg5));
    RETURN_ON_ERROR(esp_loader_read_register(((esp_loader_config_t*)config), efuse_word_addr(efuse_base, 3), &reg3));
...
static esp_loader_error_t spi_config_esp32xx(void *config, uint32_t efuse_base, uint32_t *spi_config)
{
    *spi_config = 0;
    ...
    uint32_t reg1, reg2;
    RETURN_ON_ERROR(esp_loader_read_register(((esp_loader_config_t*)config), efuse_word_addr(efuse_base, 18), &reg1));
    RETURN_ON_ERROR(esp_loader_read_register(((esp_loader_config_t*)config), efuse_word_addr(efuse_base, 19), &reg2));
